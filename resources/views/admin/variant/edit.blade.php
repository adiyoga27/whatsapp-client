                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control @error('category_id') is-invalid @enderror"
                                                ...
                                                @foreach ($product as $val)
                                                    <option value="{{ $val->id }}"
                                                        {{ old('product_id') == $val->id ? 'selected' : ($val->id == $variant->id ? 'selected' : '') }}>
                                                        {{ $val->name }}</option>
                                                @endforeach
                                    ...
                                    <div class="col-lg">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                                name="name" value="{{ old('name') ? old('name') : $variant->name }}"
                                                id="product-name" placeholder="Enter name" required>
                                            @error('name')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
